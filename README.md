...
- **Поддерживаемые протоколы:** VLESS, Trojan, Hysteria2, TUIC, Anytls.
...
1. Выберите один из вариантов:    
   Вариант А. **Подготовьте файл с URLs подписок:**
   ...
   Вариант Б. **Подготовьте текстовый файл(ы), содержащий строки конфигураций** прокси-серверов в поддерживаемых форматах:
...
2. **Поменяйте дополнительные настройки в файле `config.ini`, если необходимо:**
...
5. **Лог-файл с описанием подробностей работы программы смотрите в папке `logs`**.
...
- Конфигурации Hysteria2, TUIC и Anytls не проверяются на фактическую доступность, и фильтруются из подписок "как есть". Вместо значения времени задержки в строках комментариев этих конфигураций указано `?ms`.
