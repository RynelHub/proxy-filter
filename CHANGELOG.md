...
### Added
- Обработка конфигураций AnyTLS.
- Настраиваемый fingerprint TLS для коррекции обрабатываемых конфигураций.
- Настраиваемое время жизни кэша.

### Fixed
- Доработан функционал преобразования конфигураций из YAML в TXT.

### Removed
- Удалены неактуальные пользовательские настройки.
...
### Added
- Выбор режима работы: загрузка подписок из интернета или обработка локальных файлов.
- Локальная работа с базами данных GeoLite2 от Maxmind (требуется предварительная загрузка).
- Определение номера подсети ASN и ее владельца для IP/домена прокси и SNI.
- Кэширование состояния загруженных подписок и протестированных конфигураций.
- Автоматическая актуализация состояния кэша.
- Статистические отчеты о состоянии кэша в формате csv.
- Выбор режима DNS запросов: DNS-over-HTTPS, public, local. 
- Обработка конфигураций Hysteria2 и TUIC.
- Автоматическая проверка и коррекция обрабатываемых конфигураций.
- Тестирование доступности обрабатываемых конфигураций (Ping, TLS Handshake).
- Прогресс-бар для визуализации процесса обработки конфигураций.

### Changed
- Алгоритм работы с подписками и конфигурациями преобразован в асинхронный.
